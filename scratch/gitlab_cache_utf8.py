...
def add_promoter(name: str, zone: str, phone: str, active: int = 1):
    ...
    try:
        ...
        raise ValueError(f"El promotor '{name}' ya est├í registrado.")
    except Exception as e:
...
def update_promoter(pid: int, name: str, zone: str, phone: str, active: int):
    ...
    try:
        ...
        raise ValueError(f"El nombre '{name}' ya est├í en uso por otro promotor.")
    except Exception as e:
...
def is_first_session_of_day(phone: str, window_seconds: int = 180) -> bool:
    """Indica si es el primer contacto del d├¡a para el n├║mero.

    Devuelve True para el primer mensaje del d├¡a y para los mensajes
    posteriores que lleguen dentro de `window_seconds` desde ese primero
    (as├¡ el usuario puede navegar el "cierre de ayer" unos minutos).
    ...
    try:
        ...
        """)
        # Migraci├│n: asegurar la columna en tablas antiguas.
        try:
        ...
        if not row or row[0] != today:
            # Primer mensaje del d├¡a: inicia la ventana.
            cursor.execute(
        ...
        # Ya interactu├│ hoy: ┬┐sigue dentro de la ventana inicial?
        try:
...
def add_coordinator(name: str, cedula: str, role: str, zone: str, phone: str, active: int = 1):
    ...
    try:
        ...
        raise ValueError(f"El coordinador '{name}' ya est├í registrado.")
    except Exception as e:
...
def update_coordinator(cid: int, name: str, cedula: str, role: str, zone: str, phone: str, active: int):
    ...
    try:
        ...
        raise ValueError(f"El nombre '{name}' ya est├í en uso por otro coordinador.")
    except Exception as e:
...
def add_administrator(name: str, cedula: str, phone: str, active: int = 1):
    ...
    try:
        ...
        raise ValueError(f"El administrador '{name}' o la c├®dula '{cedula}' ya est├í registrado.")
    except Exception as e:
...
def update_administrator(aid: int, name: str, cedula: str, phone: str, active: int):
    ...
    try:
        ...
        raise ValueError(f"El nombre '{name}' o c├®dula '{cedula}' ya est├í en uso por otro administrador.")
    except Exception as e:
