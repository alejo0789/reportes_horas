...
def get_whatsapp_query(
    ...
    if report_type == "products":
        ...
        for p_name in all_products:
            ...
            pref = "" if is_count_based else "$"
            if is_past_day:
                # Recopilatorio de ayer: sin mensual, con parcial, sin venta mes, faltante en lugar de total.
                _v_pro = venta_mes_p.get(p_name, 0.0)
                _mp_pro = meta_parcial_p.get(p_name, 0.0)
                _mf_pro = meta_full_p.get(p_name, 0.0)
                _faltante_mes_pro = max(0.0, _mf_pro - _v_pro)
                _parcial_pro = (_v_pro / _mp_pro * 100.0) if _mp_pro > 0 else (100.0 if _v_pro > 0 else 0.0)
                _e_pro = lambda p: "🟢" if p >= 95 else "🔴"
                msg += f"• 📦 *{p_name}*\n"
                msg += f"  🗓️ Día {e_dia}{p_compliance:.1f}% · 📆 Parcial {_e_pro(_parcial_pro)}{_parcial_pro:.1f}%\n"
                msg += f"  ↳ Día → Meta {pref}{round(p_goal):,} · Falta {pref}{round(p_faltante):,}\n"
                msg += f"  ↳ Mes → Meta a hoy {pref}{round(_mp_pro):,} · Faltante mes {pref}{round(_faltante_mes_pro):,}\n\n"
            else:
                # Hoy: formato original + % parcial del mes.
                msg += f"• 📦 *{p_name}* ({e_dia} *{p_compliance:.1f}%*)\n"
                msg += f"  ↳ Meta del Día: {pref}{round(p_goal):,}\n"
...
# ==========================================
# ENDPOINT PARA TABLA DE CUMPLIMIENTO (IMAGEN Y DATOS)
# ==========================================
...
@app.get("/api/compliance/image")
def get_compliance_image(date: Optional[str] = Query(None, description="Fecha YYYY-MM-DD")):
    """
    Renderiza y retorna la imagen de la tabla de cumplimiento.
    """
    from fastapi.responses import FileResponse
    from backend.image_generator import generate_compliance_image
    
    report_date = date or datetime.now().strftime("%Y-%m-%d")
    try:
        image_path = generate_compliance_image(report_date)
        if os.path.exists(image_path):
            return FileResponse(image_path, media_type="image/png", filename=f"resumen_{report_date}.png")
        else:
            raise HTTPException(status_code=500, detail="Error al generar la imagen")
    except Exception as e:
        logger.error(f"Error generando imagen de cumplimiento: {e}")
