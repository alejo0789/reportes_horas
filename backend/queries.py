...
PAGOS_BETPLAY = """
SELECT P.*, U.NUM_IDENTIFICACION
FROM GANA_SIGA.SIGT_PAGOS P
LEFT JOIN GANA_MAESTROS.MAET_USUARIOS U
    ON U.IDE_USUARIO = P.IDE_USUARIO
WHERE P.IDE_PRODUCTO = 17288
  AND TRUNC(P.FEC_PAGO) = TRUNC(SYSDATE)
"""

RECARGAS_BETPLAY = """
SELECT R.*, U.NUM_IDENTIFICACION
FROM GANA_SIGA.SIGT_RECARGAS R
LEFT JOIN GANA_MAESTROS.MAET_USUARIOS U
    ON U.IDE_USUARIO = R.IDE_USUARIO
WHERE R.IDE_PRODUCTO = 17287
  AND TRUNC(R.FEC_VENTA) = TRUNC(SYSDATE)
"""

PAGOS_BETPLAY_COMPLETO = """
WITH params AS (
  SELECT
    TO_DATE(:desde, 'YYYY-MM-DD HH24:MI:SS') AS desde,
    TO_DATE(:hasta, 'YYYY-MM-DD HH24:MI:SS') AS hasta
...
SELECT
    ...
    SV.IDE_CIUDAD        AS "Cod. Ciudad",
    CIUDAD.NOM_CIUDAD    AS "Ciudad",
    OFICINA.IDE_SUBZONA  AS "Cod. Subzona",
    SUBZONA.NOM_SUBZONA  AS "Subzona",
    SUBZONA.IDE_ZONA     AS "Cod. Zona",
    ZONA.NOM_ZONA        AS "Zona",
    SV.IDE_OFICINA       AS "Cod. Oficina",
    OFICINA.NOM_OFICINA  AS "Oficina",
    SV.IDE_SITIOVENTA    AS "Cod. Sitio",
    SV.NOM_SITIOVENTA    AS "Sitio de venta",
    SV.IDE_TIPO_SITIO    AS "Cod. Tipo SV",
    TSV.DES_TIPO_SITIO   AS "Tipo SV",
    SV.DIRECCION,
    SV.ACTIVO            AS "Sitio Activo",
    SV.FEC_INGRESO       AS "Fecha Ingreso",
    SV.FEC_RETIRO        AS "Fecha Retiro",
    SV.IDE_CATEGORIA     AS "Cod. Nivel SV",
    SV.REG_MERCANTIL     AS "Registro Mercantil",
    SV.CX,
    SV.CY
FROM GANA_SIGA.SIGT_PAGOS P
LEFT JOIN GANA_MAESTROS.MAET_USUARIOS U
    ON U.IDE_USUARIO = P.IDE_USUARIO
LEFT JOIN GANA_MAESTROS.MAET_SITIOSVENTA SV
    ON SV.IDE_SITIOVENTA = P.IDE_SITIOVENTA
LEFT JOIN GANA_MAESTROS.MAET_CIUDADES CIUDAD
    ON CIUDAD.IDE_CIUDAD = SV.IDE_CIUDAD
LEFT JOIN GANA_MAESTROS.MAET_TIPOS_SITIOVENTA TSV
    ON TSV.IDE_TIPO_SITIO = SV.IDE_TIPO_SITIO
...
LEFT JOIN GANA_MAESTROS.MAET_USUARIOS U
    ON U.IDE_USUARIO = R.IDE_USUARIO
LEFT JOIN GANA_MAESTROS.MAET_SITIOSVENTA SV
    ON SV.IDE_SITIOVENTA = R.IDE_SITIOVENTA
LEFT JOIN GANA_MAESTROS.MAET_CIUDADES CIUDAD
    ON CIUDAD.IDE_CIUDAD = SV.IDE_CIUDAD
LEFT JOIN GANA_MAESTROS.MAET_TIPOS_SITIOVENTA TSV
    ON TSV.IDE_TIPO_SITIO = SV.IDE_TIPO_SITIO
...
, params pr
WHERE R.IDE_PRODUCTO = 17287
  AND R.FEC_VENTA >= pr.desde
  AND R.FEC_VENTA <  pr.hasta
"""
